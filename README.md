...
- `/bearcade:config <gamename>`:管理员打开指定游戏的**运行时配置界面**(全部 12 个小游戏均已接入,对局进行中禁止修改);
