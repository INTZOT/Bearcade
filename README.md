...
**当前状态**:首个小游戏 **Gomoku(五子棋)** 已完整可运行——大厅入房、随机黑白、放置方块落子、五连判定、结算回大厅、场地重置、强制中断均验证通过;另有可直接复制开发的小游戏模板包。
...
- 模板维度**不需要常加载区域**(`createFromWorld` 可读取未加载区块);常加载只用于房间游玩区;
...
| GuessNBuild-建筑猜猜乐 | `GuessNBuild-建筑猜猜乐/` | 建筑猜猜乐:3~16 人回合制,建筑者创造建造/猜测者旁观聊天答题,题库表单管理,侧边栏计分,目标分随人数 |
