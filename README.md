...
**当前状态**:**Gomoku(五子棋)**、**GuessNBuild(建筑猜猜乐)第一版**、**BridgeWar(急速战桥)**、**PigCatcher(猪猪争夺战)** 均已完整可运行——Gomoku 含随机黑白、放置方块落子、五连判定;GuessNBuild 含 3\~16 人回合制建筑猜谜、聊天答题、题库表单、每玩家 HUD 计分;BridgeWar 含红蓝两队核心区得分、回合制搭拆桥、队伍装备配置;PigCatcher 含四队驱赶中立猪进核心区、钓鱼竿解拴/拴绳拖拽/核心区引力场、计时结算、队伍装备实体储存;**Collapse(豆腐渣地板)** 玩法已实现(多层地板塌陷、PVP 对抗、掉虚空淘汰与 Camera 跟随观战),场地坐标当前为占位值,按包内 README 制作模板场地后即可运行;**Werewolf(天黑请闭眼)** 玩法已实现(6\~10 人昼夜行动、身份分配、投票与遗言浮空字),包内已附带旧版围桌场地结构,按 README 载入模板维度后即可运行;**Pillars(幸运之柱)** 玩法已实现(双环基岩柱大乱斗、随机物品、最后存活/击杀数结算);**Go(围棋)** 与 **CChess(中国象棋)** 玩法已完整实现(19×19 提子/劫/计时/计目、停一手终局确认;车马炮象士兵、将军/困毙/绝杀、记谱、求和/认输,均含俯瞰视角),按包内 README 搭建模板棋盘后即可运行;**HungerGame(饥饿游戏)** 玩法已实现(FFA 六阶段、物资箱热刷新、击杀/观战,512² 死场景,首次部署需 `/bearcade:tmp ap hungergame`);**Knockback(击退战)** 与 **LabEscape(实验室逃脱)** 玩法已实现(中央高台击退混战、圆环挖柱竞速),场地坐标为占位值,按包内 README 建好模板后即可运行;**Mahjong(麻将)** 已实现牡丹江麻将预设(112 张牌、吃碰杠听胡、宝牌/换宝、计分、流局/下一局、离线托管);**FrozenFloor(冰冻地板)** 玩法已实现(环形蓝冰场无限雪球互怼、地板每 30 秒融化收缩、虚空淘汰与观战、最后存活者获胜,`/bearcade:ffbuild` 生成地图);**NewYearPig(杀年猪)** 玩法已实现(45×45 地皮限时刷鸡/猪/羊、分值权重与受惊逃跑、特殊事件,`/bearcade:newyearpig_buildmap` 生成地皮);**Studio(工作室)** 玩法已实现(7 回合合成交付、货架原材料与工作台/熔炉、前三名得分与题库管理,`/studio:build` 生成地图);**Ctf(夺旗)** 玩法已实现(红蓝两队夺旗、经济与物品商店、TNT 与羊毛搭桥、生命恢复),场地坐标为模板占位值,按包内 README 建好模板场地后即可运行;另有 **SND5(剑与消亡V)** 骨架包(玩法待定),以及可直接复制开发的小游戏模板包与开发者工具包(悬浮公告 /btd、物品属性编辑 /cis)。
...
| pillars-幸运之柱 | `pillars-幸运之柱/` | 幸运之柱(可运行):双环基岩柱大乱斗、随机物品、最后存活/击杀数结算、死亡旁观、掉落物模拟 |
| Go-围棋 | `Go-围棋/` | 围棋(可运行):19×19 棋盘、提子/劫/计时/计目、停一手终局确认、俯瞰落子 |
| CChess-中国象棋 | `CChess-中国象棋/` | 中国象棋(可运行):完整规则(车/马/炮/象/士/兵、将军/困毙/绝杀、记谱、求和/认输)、第三人称俯瞰 |
| HungerGame-饥饿游戏 | `HungerGame-饥饿游戏/` | 饥饿游戏(可运行):FFA 六阶段(冻结/保护/PVP/死斗/扣血保底)、物资箱热刷新、击杀记录与观战、512² 死场景 |
| Knockback-击退战 | `Knockback-击退战/` | 击退战(可运行,场地待建):中央高台得分、强/中/弱击退木棍、定时结算 |
| LabEscape-实验室逃脱 | `LabEscape-实验室逃脱/` | 实验室逃脱(可运行,场地待建):圆环挖柱竞速、中央塌陷区、完赛排名与观战、`/labescape:build` 建图 |
| Mahjong-麻将 | `Mahjong-麻将/` | 麻将(牡丹江麻将已实现):112 张牌、吃/碰/杠/听/胡、宝牌/换宝、计分、流局/下一局、离线托管、副露实体摆放 |
| FrozenFloor-冰冻地板 | `FrozenFloor-冰冻地板/` | 冰冻地板(可运行):环形蓝冰场无限雪球、地板每 30 秒融化收缩、虚空淘汰与观战、最后存活者获胜,4 房/2~12 人/派对可用,`/bearcade:ffbuild` 建图 |
| NewYearPig-杀年猪 | `NewYearPig-杀年猪/` | 杀年猪(可运行):45×45 地皮限时刷鸡/猪/羊、分值/生成权重/受惊逃跑可配、特殊事件与 -50% 动物,4 房/2~8 人/派对可用,`/bearcade:newyearpig_buildmap` 建地皮 |
| Studio-工作室 | `Studio-工作室/` | 工作室(可运行):7 回合合成交付、货架原材料、工作台/熔炉即时烧炼、前三名得分、题库管理,2 房/2~8 人/派对可用,`/studio:build` 建图 |
| Ctf-夺旗 | `Ctf-夺旗/` | 夺旗(玩法已实现,场地待建):红蓝两队夺旗、经济与物品商店、TNT 爆破与羊毛搭桥、生命恢复、计分板 HUD,2 房/12 人 |
...
> 记分板方案:所有小游戏已不再占用全服唯一的 Sidebar 显示槽。需要记分板的小游戏把分数写入每房间独立 objective,由 `shared/minigame-core/scoreboardHud.ts` 转成 rawtext `{score:...}` 注入每位玩家的 title;这些游戏的配对资源包(源码位于 `resource-pack/ui/`)再把 title 通道重排版为屏幕右侧垂直居中的记分板,因此多房间/多游戏同时运行互不覆盖。**当前接入该方案**:BridgeWar、Collapse、Ctf、FrozenFloor、Gomoku、GuessNBuild、HungerGame、LabEscape、Mahjong、NewYearPig、PigCatcher、SND5、Studio、Template;Go / CChess / Werewolf 用 actionbar 呈现信息,pillars 不使用 title 通道(无配对 HUD 资源包),Knockback 无记分板 HUD。
