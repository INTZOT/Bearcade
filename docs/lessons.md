...
## 8. 模拟玩家(SimulatedPlayer)实测经验

> 2026-08-15,源自 Toolkit `/spm` 功能(生成/列表/删除,凑开局人数用)。

- **创建**:`@minecraft/server-gametest` 模块级 `spawnSimulatedPlayer({dimension, x, y, z}, name, gameMode)` 可在**普通世界**直接生成(不依赖 gametest 测试结构);`Test.spawnSimulatedPlayer` 仅测试环境可用。模块需在 manifest `dependencies` 声明(`@minecraft/server-gametest` 1.0.0-beta),esbuild EXTERNALS 同步登记;
- **识别**:`Player` 类型没有 `isSimulated` 属性;生成后打自定义 tag(如 `bearcade:simulated`)区分;`world.getAllPlayers()` 会包含模拟玩家,按 tag 过滤即可列表/删除;
- **删除**:用 `SimulatedPlayer.remove()`(普通世界唯一方式;`Test.removeSimulatedPlayer` 仅测试环境)。**引擎怪癖:`remove()` 会抛异常但实际已移除**——不要直接把异常当失败提示,延迟验证实际状态(实体是否还在)后再提示;
- **事件行为**:模拟玩家是 Player 实体,房间人数统计(`getPlayers()`)会计入,可凑开局人数;实测其不触发/不参与玩家进服类事件处理(如 Core 的断线重连初始化),无需对既有玩家逻辑做特殊过滤;
- 命令/表单回调受限上下文:生成与表单操作需 `system.run` 延迟(与 §1.1 一致)。
