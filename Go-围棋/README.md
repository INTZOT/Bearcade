...
- **劫**:上一步单子被提的交叉点禁落(简单劫);
- **停一手**:开局发放纸张(槽位锁定,**固定快捷栏第 9 格**防误触),**使用纸张**即停一手(仅轮到你时有效);双方连续停手 → 弹窗**终局确认**:双方都确认才计目终局,任一取消/关闭弹窗或 60 秒超时则清空停手状态继续对局(最后一个停手方行棋),确认期间落子/停手/计时均暂停;
- **计目**:领地(单色完全包围的空域)+ 提子数,黑贴 5.5 目(可配置);空域同时邻接双方不计;
- **计时**:每方 60 分钟局时(可配置),当前玩家计时,超时判负;
- **认输**:当前持棋玩家**丢弃手中的棋子物品**即认输;离开房间/断线视为认输(断线=退出契约);
- **俯瞰视角**:开局发放槽位锁定的望远镜(**固定快捷栏第 8 格**),对局中使用望远镜在棋盘正上方俯瞰视角与普通视角间切换——原生 `minecraft:free` 相机定位到棋盘中心正上方 `boardY + 1 + overviewHeight`(高度可配置),视野锁定 60(`camera.setFov`,退出时自动还原);俯瞰中本体视线强制水平(控制方案特性),**手持棋子 use(右键)= 在自己脚下最近的交叉点落子**(canPlace/itemUse/itemStartUseOn 多事件源统一入口,同 tick 去重;引擎落点被取消,由脚本放置并扣减棋子;aim assist 已注册激活但本版本实测不产生锁定框,仅作视觉辅助);**选中格提示**:俯瞰中每 0.25s 在玩家脚下格上表面用 `balloon_gas_particle` 粒子画正方形框标记落点;
