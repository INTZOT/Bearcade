...
runtime = new MinigameRuntime(

  {
    ...
    // 死场景:世界加载不重置房间(场地在存档中),首次部署手动 /bearcade:tmp ap hungergame

    resetRoomsOnLoad: false,

    // 死场景:对局结束也不重建场地(由 onBeforeReset 清理掉落物/箱子),避免 512² 每局重放

    resetArenaOnGameEnd: false,
