...
- **俯瞰视角**:开局发放槽位锁定的望远镜,对局中使用望远镜在棋盘正上方俯瞰视角与普通视角间切换——原生 `minecraft:free` 相机定位到棋盘中心正上方 `boardY + 1 + overviewHeight`,配合 `/controlscheme set camera_relative` 用鼠标/摇杆转动视野(高度可配置);进入俯瞰时玩家本体俯仰压到 90°(落子射线跟随玩家头而非相机),**右键=在自己脚下最近的交叉点落子**(引擎落点被取消,由脚本放置并扣减棋子);
