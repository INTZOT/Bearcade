...
- **俯瞰视角**:开局发放槽位锁定的望远镜,对局中使用望远镜在棋盘正上方俯瞰视角与普通视角间切换——原生 `minecraft:free` 相机定位到棋盘中心正上方 `boardY + 1 + overviewHeight`(高度可配置);俯瞰中本体视线强制水平(控制方案特性),**手持棋子 use(右键)= 在自己脚下最近的交叉点落子**(canPlace/itemUse/itemStartUseOn 多事件源统一入口,同 tick 去重;引擎落点被取消,由脚本放置并扣减棋子;aim assist 已注册激活但本版本实测不产生锁定框,仅作视觉辅助);
